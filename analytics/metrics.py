...
from config.settings import CO2_IDLE_RATE, CO2_MOVING_RATE, MAX_VEHICLES
...
class MetricsCollector:
    ...
    def co2_total(self) -> float:
        ...
        return self.co2_idle + self.co2_moving

    def record_emissions(self, idle_count: int, moving_count: int):
        """Record CO2 emissions for one frame."""
        self.co2_idle += idle_count * CO2_IDLE_RATE
        self.co2_moving += moving_count * CO2_MOVING_RATE
    ...
    def generate_report(self) -> str:
        lines = [
            ...
            f"  CO₂ emitted (idle) : {self.co2_idle:.2f} kg",
            f"  CO₂ emitted (move) : {self.co2_moving:.2f} kg",
            f"  CO₂ total          : {self.co2_total:.2f} kg",
            "═" * 50,
        ]
        return "\n".join(lines)
